...
Downstream correctness can mask upstream failure.

A workflow may follow its rules, a model may satisfy its instruction, and an execution layer may commit cleanly — yet the whole sequence can still be structurally invalid if the human, role, responsibility, or institutional context was misrecognized before legitimacy formed.

Recognition Integrity establishes the precondition.

Legitimacy forms only after that condition is satisfied.
